...
class MyGameHandler : public IEntity
{
public:
    MyGameHandler(const EntityTransform& transform = { {0,0}, 0 }, int id = -1) : IEntity(id, transform)
    {}
    void on_create() override { m_gameHandler = &(rcm::get_gameHandler()); }
    void on_update() override {}
    void on_late_update() override;
    ...
    int& get_activeEnemies() { return m_activeEnemies; }
private:
...
MyProjectile::MyProjectile(const EntityTransform& transform, int id) : IEntity(id, transform)
{
...
MyPlayer::MyPlayer(const EntityTransform& transform, int id) : IEntity(id, transform)
{
...
void MyPlayer::on_hit(EntityType otherEntity)
{
...
MyFallenEnemy::MyFallenEnemy(const EntityTransform& transform, int id) : IEntity(id, transform)
{
...
MyEnemy::MyEnemy(const EntityTransform& transform, int id) : IEntity(id, transform)
{
...
void MyEnemy::on_hit(EntityType otherEntity)
{
    if (otherEntity == EntityType::projectile)
    {
...
MySpawner::MySpawner(const EntityTransform& transform, int frame_number, const unsigned char* ids) : IEntity(ids[0], transform),
...
void MyGameHandler::on_late_update()
{
    ...
    else
    {
        ...
        {
            m_gameHandler->set_text_ui("Goal Reached!", TextVerticalAlignment::Center, TextHorizontalAlignment::Center, 50);
        }
        else
        {
            std::string text("<3 = ");
            text.append(std::to_string(m_player->get_playerHearts())).append("/").append(std::to_string(m_heartsCap));
